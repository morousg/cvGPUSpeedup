...
bool benchmark_num_instructions_per_operation(cv::cuda::Stream& cv_stream, bool enabled) {
    ...
    if (enabled) {
        ...
        if (!passed) {
            if (!exception) {
                std::stringstream ss;
                ss << "benchmark_num_instructions_per_operation<" << cvTypeToString<CV_8UC3>() << ", " << cvTypeToString<CV_32FC3>();
...
int launch() {
    ...
    bool correct = benchmark_num_instructions_per_operation(cv_stream, true);
