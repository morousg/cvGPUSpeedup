...
bool benchmark_vertical_fusion_loopAdd(size_t NUM_ELEMS_X, size_t NUM_ELEMS_Y, cv::cuda::Stream& cv_stream, bool enabled) {
    ...
    if (enabled) {
        ...
        const cv::Scalar val_add = one.val_add;
        try {
            ...
            const OutputType val{ cvGS::cvScalar2CUDAV<CV_TYPE_O>::get(val_add) };

            // cvGPUSpeedup
            const auto dFunc = Add<OutputType>::build(val).then(Add<OutputType>::build(val));
        ...
        if (!passed) {
            if (!exception) {
                std::stringstream ss;
                ss << "benchmark_vertical_fusion_loopAdd<" << cvTypeToString<CV_TYPE_I>() << ", " << cvTypeToString<CV_TYPE_O>();
...
bool launch_benchmark_vertical_fusion_loopAdd(const size_t NUM_ELEMS_X, const size_t NUM_ELEMS_Y, std::index_sequence<Is...> seq, cv::cuda::Stream cv_stream, bool enabled) {
    bool passed = true;

    int dummy[] = { (passed &= benchmark_vertical_fusion_loopAdd<CV_TYPE_I, CV_TYPE_O, (Is+1)>(NUM_ELEMS_X, NUM_ELEMS_Y, cv_stream, enabled), 0)... };
    (void)dummy;
...
int launch() {
    ...
    results["launch_benchmark_vertical_fusion_loopAdd"] = true;
    constexpr auto iSeq = std::make_index_sequence<NUM_EXPERIMENTS>{};
#define LAUNCH_TESTS(CV_INPUT, CV_OUTPUT) \
    results["launch_benchmark_vertical_fusion_loopAdd"] &= launch_benchmark_vertical_fusion_loopAdd<CV_INPUT, CV_OUTPUT>(NUM_ELEMS_X, NUM_ELEMS_Y, iSeq, cv_stream, true);
