...
bool test_npp_batchresize_x_split3D(size_t NUM_ELEMS_X, size_t NUM_ELEMS_Y, cudaStream_t &compute_stream, bool enabled)
{
	...
	if (enabled)
	{
		...
		try
		{
			...
			{
				const fk::Ptr2D<uchar3> d_cropped = d_input.crop2D(fk::Point(crop_i, crop_i), fk::PtrDims<fk::_2D>{CROP_W, CROP_H, d_input.ptr().dims.pitch});
		...
		//	d_tensor_output.step = up.width * up.height * sizeof(float);
			...
			for (int crop_i = 0; crop_i < BATCH; crop_i++)
			{
				...
				dstcropsdesc[crop_i].pData = (uchar *)d_output_crop_npp[crop_i].data();
				dstcropsdesc[crop_i].nStep = d_output_crop_npp[crop_i].step;
		...
		compute_stream.waitForCompletion();
