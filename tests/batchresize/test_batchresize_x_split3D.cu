...
bool test_batchresize_x_split3D_OCVBatch(int NUM_ELEMS_X, int NUM_ELEMS_Y, cv::cuda::Stream &cv_stream, bool enabled) {
  ...
  if (enabled) {
    ...
    try {
      ...
      cv::cuda::GpuMat d_resize_output(BATCH, up.width * up.height, CV_TYPE_O);
      d_resize_output.step = up.width * up.height * CV_MAT_CN(CV_TYPE_O) * sizeof(BASE_CUDA_T(CV_TYPE_O));
      ...
      cv::cuda::GpuMat crop_32F(cv::Size(CROP_WIDTH, CROP_HEIGHT), CV_TYPE_O);
      for (int crop_i = 0; crop_i < BATCH; crop_i++) {
      ...
      for (int crop_i = 0; crop_i < BATCH; crop_i++) {
        ...
        cv::cuda::resize(crop_32F, d_resized_array[crop_i], up, 0., 0., cv::INTER_LINEAR, cv_stream);
      }

      cv::cuda::multiply(d_resize_output, val_alpha, d_temp, 1.0, -1, cv_stream);
      ...
      if constexpr (CV_MAT_CN(CV_TYPE_O) == 3 && correctDept) {
        ...
        cvGS::executeOperations(cv_stream, resizeDF, cvGS::cvtColor<cv::COLOR_RGB2BGR, CV_TYPE_O>(),
                                cvGS::multiply<CV_TYPE_O>(val_alpha), cvGS::subtract<CV_TYPE_O>(val_sub),
                                cvGS::divide<CV_TYPE_O>(val_div), cvGS::split<CV_TYPE_O>(d_tensor_output, up));
      } else if constexpr (CV_MAT_CN(CV_TYPE_O) == 4 && correctDept) {
        cvGS::executeOperations(cv_stream, cvGS::resize<CV_TYPE_I, cv::INTER_LINEAR, BATCH>(crops, up, BATCH),
                                cvGS::cvtColor<cv::COLOR_RGBA2BGRA, CV_TYPE_O>(), cvGS::multiply<CV_TYPE_O>(val_alpha),
                                cvGS::subtract<CV_TYPE_O>(val_sub), cvGS::divide<CV_TYPE_O>(val_div),
                                cvGS::split<CV_TYPE_O>(d_tensor_output, up));
      } else {
        cvGS::executeOperations(cv_stream, cvGS::resize<CV_TYPE_I, cv::INTER_LINEAR, BATCH>(crops, up, BATCH),
                                cvGS::convertTo<CV_TYPE_I, CV_TYPE_O>(), cvGS::multiply<CV_TYPE_O>(val_alpha),
                                cvGS::subtract<CV_TYPE_O>(val_sub), cvGS::divide<CV_TYPE_O>(val_div),
                                cvGS::split<CV_TYPE_O>(d_tensor_output, up));
      }
      STOP_CVGS_BENCHMARK
    ...
    if (!passed) {
      if (!exception) {
        std::stringstream ss;
        ss << "test_batchresize_x_split3D_OCVBatch<" << cvTypeToString<CV_TYPE_I>() << ", "
...
bool test_batchresize_x_split3D_OCVBatch(int NUM_ELEMS_X, int NUM_ELEMS_Y, std::index_sequence<Is...> seq,
                                         cv::cuda::Stream &cv_stream, bool enabled) {
  bool passed = true;
  int dummy[] = {(passed &= test_batchresize_x_split3D_OCVBatch<CV_TYPE_I, CV_TYPE_O, batchValues[Is]>(
                      NUM_ELEMS_X, NUM_ELEMS_Y, cv_stream, enabled),
                  0)...};
  return passed;
}
...
template <int CV_TYPE_I, int CV_TYPE_O, size_t... Is>
bool test_batchresize_x_split3D(const size_t NUM_ELEMS_X, const size_t NUM_ELEMS_Y, std::index_sequence<Is...> seq,
                                cv::cuda::Stream cv_stream, bool enabled) {
  bool passed = true;
  int dummy[] = {(passed &= test_batchresize_x_split3D<CV_TYPE_I, CV_TYPE_O, batchValues[Is]>(NUM_ELEMS_X, NUM_ELEMS_Y,
                                                                                              cv_stream, enabled),
                  0)...};
  return passed;
}
