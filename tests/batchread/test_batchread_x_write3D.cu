...
template <int CV_TYPE_I, int CV_TYPE_O, int BATCH>
bool test_batchread_x_write3D_only_HorizontalFusion(size_t NUM_ELEMS_X, size_t NUM_ELEMS_Y, cv::cuda::Stream& cv_stream, bool enabled) {
    std::stringstream error_s;
    bool passed = true;
    bool exception = false;
...
int launch() {
    ...
    results["test_batchread_x_write3D_only_HorizontalFusion"] = true;
...
#define LAUNCH_TESTS(CV_INPUT, CV_OUTPUT) \
    results["test_batchread_x_write3D_only_HorizontalFusion"] &= launch_test_batchread_x_write3D_only_HorizontalFusion<CV_INPUT, CV_OUTPUT>(NUM_ELEMS_X, NUM_ELEMS_Y, iSeq, cv_stream, true);
